...
describe SportNginWatir::Browser do
  ...
  describe ".new" do
    it "passes the args to webdriver" do
      ...
      SportNginWatir::Browser.new(:firefox, :foo)
    end
    ...
    it "takes a Driver instance as argument" do
      ...
      expect { SportNginWatir::Browser.new(mock_driver) }.to_not raise_error
    end

    it "raises ArgumentError for invalid args" do
      expect { SportNginWatir::Browser.new(Object.new) }.to raise_error(ArgumentError)
    end
  end

  describe "#execute_script" do
    before { browser.goto SportNginWatirSpec.url_for("definition_lists.html") }

    it "wraps elements as SportNginWatir objects" do
      returned = browser.execute_script("return document.body")
      expect(returned).to be_kind_of(SportNginWatir::Body)
    end

    it "wraps elements in an array" do
      list = browser.execute_script("return [document.body];")
      expect(list.size).to eq 1
      expect(list.first).to be_kind_of(SportNginWatir::Body)
    end

    it "wraps elements in a Hash" do
      hash = browser.execute_script("return {element: document.body};")
      expect(hash['element']).to be_kind_of(SportNginWatir::Body)
    end
    ...
    it "wraps elements in a deep object" do
      ...
      expect(hash['elements'].first).to be_kind_of(SportNginWatir::Body)
      expect(hash['body']['element']).to be_kind_of(SportNginWatir::Body)
    end
  end
  ...
  describe "#send_key{,s}" do
    ...
    it "sends keys to a frame" do
      browser.goto SportNginWatirSpec.url_for "frames.html"
  ...
  it "raises an error when trying to interact with a closed browser" do
    b = SportNginWatirSpec.new_browser
    b.goto SportNginWatirSpec.url_for "definition_lists.html"
    b.close

    expect { b.dl(:id => "experience-list").id }.to raise_error(SportNginWatir::Exception::Error, "browser was closed")
  end
  ...
  describe '#screenshot' do
    it 'returns an instance of of SportNginWatir::Screenshot' do
      expect(browser.screenshot).to be_kind_of(SportNginWatir::Screenshot)
    end
  end
end
